...
/// <summary>
/// shape工厂类，用于按需创建和管理shape实例。客户端无需了解这些shape是如何创建的，可以直接通过此工厂获取shape。
/// 支持shape实例池化，从而优化性能。
/// </summary>
[CreateAssetMenu]
public class ShapeFactory : ScriptableObject
{
    ...
    /// <summary>
    /// 根据给定的shapeID和材质ID创建一个新的shape实例，并为其设置相应的材质。
    /// </summary>
    /// <param name="shapeId">需要创建的shapeID。</param>
    /// <param name="materialId">需要为shape设置的材质ID。</param>
    /// <returns>一个创建的Shape实例，已设置材质和shapeID。</returns>
    public Shape Get(int shapeId = 0, int materialId = 0)
    {
        Shape instance;
        // 1. 根据是否回收销毁的shape实例，选择不同的实例化方式 
        if (m_recycle)
        {
            // 1 检查对象池初始化 
            if (m_pools == null)
    ...
    /// <summary>
    /// 随机生成一个shape实例，并为其随机选择一个材质。
    /// </summary>
    /// <returns>一个随机生成的Shape实例，已设置随机材质和shapeID。</returns>
    ...
    /// <summary>
    /// 将一个shape实例返回到池中，以便下次复用。如果没有启用回收机制，则销毁该实例。
    /// </summary>
    ...
    /// <summary>
    /// 初始化shape池。每个shapeID对应一个池，用于存储销毁的shape实例。
    /// 在编辑器模式下，如果已加载场景，尝试从场景中的根对象获取已销毁的shape实例并将其加入池中。
    /// 如果没有加载场景，则创建一个新的场景用于存放这些销毁的shape实例。
    /// </summary>
    private void CreatePools()
    {
        // 1. 初始化对象池，池的数量与shape预设数量相同，每个池用于存储不同shapeID对应的销毁实例。
        ...
        // 2. 如果场景未加载或不在编辑器模式下，则创建一个新的场景用于存放销毁的shape实例
    ...
    /// <summary>
    /// 工厂的唯一标识符。
    /// 此属性用于区分不同的shape工厂实例，尤其是在保存和加载游戏状态时，
    /// 确保shape能正确关联到其原始的工厂设置。
    /// </summary>
    ...
    /// <summary>
    /// 池场景来容纳所有可以回收的shape实例。工厂的所有shape都进入这个池，并且永远不应该从中移除。
    /// </summary>
