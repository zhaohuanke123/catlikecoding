...
public class OrbitCamera : MonoBehaviour
{
    ...
    {
        // 1. 获取当前向上方向和重力方向。
        var fromUp = m_gravityAlignment * Vector3.up;
        var toUp = CustomGravity.GetUpAxis(m_focusPoint);
        
        // 2. 计算插值的角度和最大角度。
        float dot = Mathf.Clamp(Vector3.Dot(fromUp, toUp), -1f, 1f);
        float angle = Mathf.Acos(dot) * Mathf.Rad2Deg;
        float maxAngle = m_upAlignmentSpeed * Time.deltaTime;
        var newAlignment = Quaternion.FromToRotation(fromUp, toUp) * m_gravityAlignment;
        ...
        {
            // 已确保仅在必要时进行插值，插值器保证在 0 - 1 范围内，因此可以使用 SlerpUnclamped
            m_gravityAlignment = Quaternion.SlerpUnclamped(m_gravityAlignment, newAlignment, maxAngle / angle);
        }
    }
